...
function SimpleDataList({ type, nameKey, items, visibleData, newButton, shouldUpdate, onDelete, onUpdate, confirmMessages }) {
          ...
          <div className="left-[1rem] z-10 mt-1 max-h-[450px] rounded-md py-1 text-base overflow-auto">
            {data.length === 0 || !data ? (
              ...
              >
                ...
                <button
                  ref={button1}
                  title={getTranslation('general_text_edit', 'Edit')}
                ...
                <button
                  ref={button2}
                  title={getTranslation('general_text_delete', 'Delete')}
                  onClick={() => {
                    confirm({
...
SimpleDataList.propTypes = {
  ...
  newButton: PropTypes.object.isRequired
}
